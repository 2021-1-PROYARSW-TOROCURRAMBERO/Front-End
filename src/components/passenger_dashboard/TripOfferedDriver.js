...
class TripOfferedDriver extends Component {
    ...
    handleOfferTrips(response,topic){
        if(topic === '/quickmobility/drivers'){
            if(!(response === "The driver selected is not available") && response ){
                ...
                if(this.state.reload){
                    Swal.fire({
                        position: 'bottom-end',
                        title: "nuevo viaje agregado del conductor: " + this.state.trips[response.length -1].username,
                        width: 600,
                        timer: 1500,
        ...
        return (
            <Grid container className={classes.gridContainer} spacing={2}>
                <SockJsClient
                    ...
                    topics={['/quickmobility/drivers','/quickmobility/passengerRequest']}
